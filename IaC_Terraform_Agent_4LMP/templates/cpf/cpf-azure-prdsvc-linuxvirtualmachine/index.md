...
This terraform module creates a Linux virtual machine and associated resources.
...
## Prerequisites
  ...
  - Keyvault Secret module to store the Linux SSH Key.
  - Disk Encryption Set module to be used by the OS disk of the Linux VM.
  ...
  - User Assigned Identity module for the Linux VM.
  ...
  - `tls_private_key` resource block to generate SSH key Pair for the Linux VM.

  [Image: Linux Virtual Machine]

## Guidance

#### Usage

- To test the use of SSH key, we have followed the below steps implemented in the .tests\deployTest\main.tf file:
  1. Used `tls_private_key` resource block to generate the SSH Keys,
  2. Referred the use of `tls_private_key` resource block in getting the value of `admin ssh public_key`in Linux VM Module,
  4. Stored the private key as a secret (named after the Virtual Machine name) in the Key Vault to be used by the user to login the Linux Virtual Machine,
  5. Additional role(`Key Vault Secrets User`) is required on the Key Vault to manage the key as Key Vault secret.
...
###### AzureRM 3.x to 4.x Upgrade Notes for Linux Virtual Machine

Product Impact -- Low

The Linux Virtual Machine module creates and manages a Network Interface resource as a core dependency. Due to this dependency relationship, changes to the Network Interface resource in the AzureRM provider 4.x upgrade directly impact this module.
...
This module has been updated to use the new 4.x compliant properties (accelerated_networking_enabled and ip_forwarding_enabled) in the Network Interface configuration.
  ...
  - Wiki link for [AzureRM 4.x Upgrade](https://gitlab.dx1.lseg.com/groups/app/app-51310/azure/prdsvc/terraform/-/wikis/Terraform-AzureRM-Provider-Upgrade:-Version-3.x-to-4.x/linuxvirtualmachine) for details on the upgrade process.
...
- `admin_username` and `username` of the `admin_ssh_key` block should have the same value, otherwise, the apply will give an error - "Destination path for SSH public keys is currently limited to its default value /home/lsegadmin/.ssh/authorized_keys due to a known issue in Linux provisioning agent." Link provided in the docs section.
- Use LSEG Golden Images stored in Shared Image Gallery to deploy Linux Virtual Machines. These Golden Images are maintained and managed by Share image gallery team. Linux Virtual Machine module takes ID of the Golden Images as Input using the Parameter `source_image_id`.
- Default size of a Linux Virtual Machine is set to `Standard_D2s_v5`.
- The Disk Encryption Set must have the `Reader` Role Assignment scoped on the Key Vault.

#### Additional Information

- This module doesn't include `patching` related arguments - `patch_mode`, `patch_assessment_mode`, `reboot_setting`, `bypass_platform_safety_checks_on_user_schedule_enabled`.
- The module has been created with `priority` as `Regular`, hence, no arguments of `SPOT` have been included - `eviction_policy`, `max_bid_prize`.
- `Secret` block is not included in the module as of now.
- In this release there's a known issue where the `public_ip_address` and `public_ip_addresses` fields may not be fully populated for Dynamic Public IP's.
- Extensions and Gallery Application are not included in this initial version of the module.
- `platform_fault_domain` and `virtual_machine_scale_set_id` both should be specified if any one of it is included in the module. As we are not creating orchestrated VMSS for this module, these have not been included.
...
| 17. |  AZU-VM-SI_010 | Virtual Machines must enable Trusted Launch (secure boot, vTPM, Integrity monitoring) when Azure Site recovery is not required | Virtual Machines must enable Trusted Launch (secure boot, vTPM, Integrity monitoring) when Azure Site recovery is not required (What) within the Configuration settings (How) to ensure the integrity and attestation of the boot process (Why) | True | False | Can be enabled using variables `vtpm_enabled = true` and `secure_boot_enabled = true`. These variables have default value of `false` and can be configured as required. The pester tested is skipped in LSEG environments as the Golden Images used cannot have trusted launch enabled with crowdstrike integrated. |
...
- [azure-prdsvc-terraform-Linuxvirtualmachine](CHANGELOG.md)
...
- [VM Size allowed with Image Gen2](https://learn.microsoft.com/en-us/azure/virtual-machines/generation-2)

### Terraform Docs

- [azurerm_network_interface](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/network_interface)
- [azurerm_Linux_virtual_machine](https://registry.terraform.io/providers/hashicorp/azurerm/3.70.0/docs/resources/linux_virtual_machine)
- [Azure Provider Public SSH Keys](https://discuss.hashicorp.com/t/azure-provider-ssh-public-keys-destination-path-restriction/40909/3)
- [TF issue with platform_fault_domain](https://github.com/hashicorp/terraform/issues/30622)
...
2. What are the required variables for setting <b>Trusted Launch</b> for the supported Golden Image?
   ...
   - Put the values as `true` for the below variables:
     - vtpm_enabled
     - secure_boot_enabled
3. How to you deploy the Linux Virtual machine using Golden Images?

   - Provide the Linux Image ID to the `source_image_id` variable instead of using `source_image_reference` for the Marketplace images.
   - List of golden images and the details can be found here [LMP Azure - Golden Images](https://lsegroup.sharepoint.com/:u:/r/sites/CloudCentral/SitePages/LMP-Azure-Golden-Images.aspx?csf=1&web=1&e=yOmWLD)
...
|------|-------------|------|---------|:--------:|
| <a name="input_admin_ssh_public_key"></a> [admin_ssh_public_key](#input_admin_ssh_public_key) | (Required) The Public Key which should be used for authentication, which needs to be at least 2048-bit and in ssh-rsa format. Changing this forces a new resource to be created. | `string` | n/a | yes |
...
| <a name="input_linux_virtual_machine_timeouts"></a> [linux_virtual_machine_timeouts](#input_linux_virtual_machine_timeouts) | (Optional) Timeout configuration for Linux Virtual Machine. Only applied when enable_timeouts is true.<br/>object({<br/>  create = "(Optional) Timeout for creating the Linux Virtual Machine. Example: '45m'"<br/>  read   = "(Optional) Timeout for reading the Linux Virtual Machine. Example: '5m'"<br/>  update = "(Optional) Timeout for updating the Linux Virtual Machine. Example: '45m'"<br/>  delete = "(Optional) Timeout for deleting the Linux Virtual Machine. Example: '45m'"<br/>}) | <pre>object({<br/>    create = optional(string)<br/>    read   = optional(string)<br/>    update = optional(string)<br/>    delete = optional(string)<br/>  })</pre> | `null` | no |
...
| <a name="input_network_interface_timeouts"></a> [network_interface_timeouts](#input_network_interface_timeouts) | (Optional) Timeout configuration for Network Interface. Only applied when enable_timeouts is true.<br/>object({<br/>  create = "(Optional) Timeout for creating the Network Interface. Example: '30m'"<br/>  read   = "(Optional) Timeout for reading the Network Interface. Example: '5m'"<br/>  update = "(Optional) Timeout for updating the Network Interface. Example: '30m'"<br/>  delete = "(Optional) Timeout for deleting the Network Interface. Example: '30m'"<br/>}) | <pre>object({<br/>    create = optional(string)<br/>    read   = optional(string)<br/>    update = optional(string)<br/>    delete = optional(string)<br/>  })</pre> | `null` | no |
...
| <a name="input_size"></a> [size](#input_size) | (Required) The SKU which should be used for this Virtual Machine. | `string` | `"Standard_D2s_v5"` | no |
...
| <a name="input_virtual_machine_extension_timeouts"></a> [virtual_machine_extension_timeouts](#input_virtual_machine_extension_timeouts) | (Optional) Timeout configuration for Virtual Machine Extensions. Only applied when enable_timeouts is true.<br/>object({<br/>  create = "(Optional) Timeout for creating the Virtual Machine Extension. Example: '30m'"<br/>  read   = "(Optional) Timeout for reading the Virtual Machine Extension. Example: '5m'"<br/>  update = "(Optional) Timeout for updating the Virtual Machine Extension. Example: '30m'"<br/>  delete = "(Optional) Timeout for deleting the Virtual Machine Extension. Example: '30m'"<br/>}) | <pre>object({<br/>    create = optional(string)<br/>    read   = optional(string)<br/>    update = optional(string)<br/>    delete = optional(string)<br/>  })</pre> | `null` | no |
...
| <a name="output_resource"></a> [resource](#output_resource) | The Linux Virtual Machine resource. |
